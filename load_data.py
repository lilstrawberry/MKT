...
class KT_Dataset(data.Dataset):
    ...
    def __init__(self, skill_neibor_pro, problem_max, problem_list, ans_list, skill_list, min_problem_num,
                 max_problem_num):
        ...
        self.skill_neibor_pro = skill_neibor_pro
    ...
    def __getitem__(self, index):
        ...
        neg_skill = copy.deepcopy(last_skill)

        pos_problem = np.zeros(last_problem.shape, dtype=int)
        neg_problem = np.zeros(last_problem.shape, dtype=int)
        max_pro = len(self.skill_neibor_pro)

        #         for i, x in enumerate(last_problem):
        #             now_neibor_list = self.skill_neibor_pro[x]

        #             node_neibor_id = now_neibor_list[np.random.randint(0, len(now_neibor_list))]

        #             pos_problem[i] = node_neibor_id

        #             neg_neibor_id = np.random.randint(0, max_pro)
        #             while neg_neibor_id in now_neibor_list:
        #                 neg_neibor_id = np.random.randint(0, max_pro)
        #             neg_problem[i] = neg_neibor_id
        #             neg_skill[i] = pro2skill[neg_neibor_id]
        ...
        pos_problem = torch.from_numpy(next_problem).to(device).long()
        neg_problem = torch.from_numpy(next_problem).to(device).long()

        # 这里改了

        last_skill = torch.from_numpy(last_skill).to(device).long()
        neg_skill = torch.from_numpy(neg_skill).to(device).long()
        ...
        return last_problem, pos_problem, neg_problem, last_ans, last_skill, next_skill, neg_skill, next_problem, next_ans, res_mask


def getLoader(skill_neibor_pro, problem_max, skill_path, path, batch_size, is_train, min_problem_num, max_problem_num):
    ...
    dataset = KT_Dataset(skill_neibor_pro, problem_max, problem_list, ans_list, skill_list, min_problem_num,
                         max_problem_num)
